...
class local_homology(BaseEstimator, TransformerMixin): #plotterMixin?
	...
	def _memberships(self):
		...
		if self.neighborhood == 'nb_neighbours':
			...
			large_neighbs = kneighbors_graph(self.X_mat, n_neighbors=min(self.size-1, 50) , metric='precomputed', include_self=False, n_jobs=self.n_jobs).toarray() #is this line already covered?
