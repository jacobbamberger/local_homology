...
class local_homology(BaseEstimator, TransformerMixin): #plotterMixin?
	...
	def __init__(self, metric = 'euclidean', neighborhood = 'nb_neighbours', neighborhood_params=None, 
		homology=None , homology_params = None, vectorizer=None,  
		n_jobs=None):
		...
		self.homology_params = homology_params
		...
		if self.homology_params is None: 
			self.homology_params = {'dimensions':(0, 1, 2)}
		if self.homology is None:
			self.homology_params['dimensions'] = sorted(self.homology_params['dimensions'])
			self.homology = VietorisRipsPersistence(metric="precomputed",collapse_edges=True, homology_dimensions=self.homology_params['dimensions'], n_jobs=self.n_jobs)#tuple(range(self.max_dim+1)))

		if self.neighborhood_params is None:
			if self.neighborhood == 'nb_neighbours':
				self.neighborhood_params = {'nb_neighbours1': 10, 'nb_neighbours2': 50}
			elif self.neighborhood == 'epsilon_ball':
				self.neighborhood_params = {'epsilon1': 0.01, 'epsilon2': 0.1}
	...
	def _memberships(self):
		...
		if self.neighborhood == 'nb_neighbours':
			large_neighbs = kneighbors_graph(self.X_mat, n_neighbors=self.neighborhood_params['nb_neighbours2'], metric='precomputed', include_self=False, n_jobs=self.n_jobs ).toarray() #USE CSR SCIPY
		elif self.neighborhood == 'epsilon_ball':
			large_neighbs = radius_neighbors_graph(self.X_mat, radius=self.neighborhood_params['epsilon2'], metric='precomputed', include_self=False, n_jobs=self.n_jobs).toarray()
		else:
			warnings.warn('Unrecognized neighbourhood method, using default.')
			large_neighbs = kneighbors_graph(self.X_mat, n_neighbors=50, metric='precomputed', include_self=False, n_jobs=self.n_jobs).toarray()
		...
		if self. neighborhood == 'nb_neighbours':
			small_neighbs = Parallel(n_jobs=self.n_jobs)(delayed(np.where)
				(loc_mat[0] <= np.partition(loc_mat[0], self.neighborhood_params['nb_neighbours1'])[self.neighborhood_params['nb_neighbours1']], 1, 0) for loc_mat in loc_mats ) #[np.where(loc_mat[0]<np.partition(loc_mat[0], self.neighborhood_params['nb_neighbours'])[self.neighborhood_params['nb_neighbours']], 1, 0) for loc_mat in loc_mats] #PARALLELIZE THIS
		elif self.neighborhood == 'epsilon_ball':
			small_neighbs = Parallel(n_jobs=self.n_jobs)(delayed(np.where)
				(loc_mat[0] <= self.neighborhood_params['epsilon1'], 1, 0) for loc_mat in loc_mats ) #[np.where(loc_mat[0] < self.neighborhood_params['epsilon'], 1, 0) for loc_mat in loc_mats] #1 is inside small neighborhood, 0 if not. What about point itself? seems like it is
		else:
			warnings.warn('Unrecognized neighbourhood method, using default.')
	...
	def plot(self, X, dimension = None):
		...
		if not(dimension is None) and dimension in self.homology_params['dimensions']:
			color = self.dim_vects[:, np.argwhere(np.array(self.homology_params['dimensions'])==dimension)[0][0]]
		else: 
			print('invalid dimension, colouring with dimension '+ str(self.homology_params['dimensions'][0]))
...
class loc_hom_pipeline(ClassifierMixin, BaseEstimator):
	def __init__(self, label_col=None, train_cols=None, metric = 'euclidean',
		neighborhood = 'nb_neighbours', neighborhood_params=None, 
		vectorizer=None, homology=None, homology_params = None,
		preprocessor=None, reducer=None, classifier=None, 
		q_feature_aug = False, n_jobs=None):

		self.metric = metric
		self.neighborhood = neighborhood
		self.neighborhood_params = neighborhood_params
		self.homology = homology
		self.homoplogy_params = homology_params
		self.n_jobs=n_jobs
		...
		self.q_feature_aug = q_feature_aug
		self.check_is_fitted = False
	...
	def feature_engineering(self,df):
		self.X = df[self.train_cols].values
		self.y = df[self.label_col].values
		self.lh = local_homology(metric=self.metric, neighborhood=self.neighborhood, neighborhood_params=self.neighborhood_params,
			vectorizer=self.vectorizer, n_jobs=self.n_jobs)

		self.dim_vec = self.lh.fit_transform(self.X)
		if self.q_feature_aug:
			self.X_aug = np.hstack((self.X,self.dim_vec))
		else:
			self.X_aug = self.X
		self.X_tr = self.preprocessor.fit_transform(self.X_aug)
		return self.X_tr, self.y

	def fit(self,df):
		self.feature_engineering(df)
		self.check_is_fitted = True
		self.classifier.fit(self.X_tr, self.y)
		#self.score = clsfr.score(X_tr, y)
		return self

	def reduce(self):
		if self.check_is_fitted:
			X_red = self.reducer.fit_transform(self.X_tr)
			return X_red, np.array(self.lh.numberize(self.dim_vec, self.numberize_basis))
		else:
			print("You need to fit the class first!")

	def predict(self,X):
		if self.check_is_fitted:
			return self.classifier.predict(X)
		else:
			rint("You need to fit the class first!")

	def fit_predict(self,df):
		return self.fit(df).predict(self.X_tr)

# def reduce_plot(self, X):
