...
class LocalKNeigborsVietorisRipsPersistence(BaseEstimator, TransformerMixin, PlotterMixin):
...
class LocalVietorisRipsPersistence(BaseEstimator, TransformerMixin, PlotterMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self._is_precomputed:
            self.X_mat_ = X # the trailing "_" was added to fix the check_is_fitted bug
        else:
            self.X_ = X # the trailing "_" was added to fix the check_is_fitted bug
        self.size = len(X)
    ...
    def _memberships(self, X):
        ...
        if self._is_precomputed:
            ...
            dist_mat = self.X_mat_
        else:
            dist_mat = cdist(X, self.X_, metric=self.metric)
        ...
        if self.neighborhood == 'nb_neighbours':
            ...
            large_neighbs = radius_neighbors_graph(
                                 ...
                                 include_self=True,
                                 n_jobs=self.n_jobs).toarray()
            # Question: Is it worth it to avoid doing .toarray()?
    ...
    def _cone_off_mat(self, loc_mat, membership_vect):
        ...
        augm_loc_mat = np.concatenate(
                                     (pre_augm_loc_mat, np.array([new_col], 
                                        dtype=float).T),
                                      axis=1)
        return augm_loc_mat
